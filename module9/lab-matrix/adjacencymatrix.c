...
void generateGraph(int* g){
    ...
    for(i=0; i < ROWS; i++){
        for(j=0; j < COLUMNS; j++){
            if(i==j){
                g[i*COLUMNS+j] = 0; //Since we don't want the node to connect to itself
            }else{
...
int nodeInDegree(int* g, int node){
	// if g is a null pointer, exit the function
    if (g == NULL) {
		exit(0);
	}
	// Since this program defines row and column to be 5, the node cannot exeed this number
	if (node < 0 || node > 4) {
		exit(0);
	}
	//since this function computes the in-dgree of a node, we want to count the number of 1 in the node column
	int count_degrees = 0;
...
int isConnected(int* g, int node1, int node2){
	...
	// Since this program defines row and column to be 5, the node cannot exeed this number
	if (node1 < 0 || node1 > 4 || node2 < 0 || node2 > 4) {
		exit(0);
	}
	//to see if node1 is connected to node2, we need to go to row node1 to see if col node2 is 1 or not
	if (g[node1 * ROWS + node2] > 0) {
		return 1;
	}
	return 0;
}
