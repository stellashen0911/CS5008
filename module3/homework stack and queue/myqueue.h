...
queue_t* create_queue(unsigned int _capacity){
	...
	myQueue->data = (int*)malloc(_capacity * (sizeof(int))); //create an empty array with capacity to store data
...
int queue_enqueue(queue_t* q, int item){
	if (queue_full(q) == 1 || q == NULL ||q->data == NULL) { //if the queue is full, or the queue is NULL or the queue data // is NULL, return -1 to indicate fails	
		return -1;
	}
//since the queue is circular, the new position of back should be back index + 1 mod capacity of queue	
	int back_position = q->back % q->capacity;
	q->data[back_position] = item;
...
int queue_dequeue(queue_t *q){
	...
	q->size = q->size - 1;
	return output; // Note: This line is a filler so the code compiles.
}
